-- Limit AT scheduled sync years to current + previous only.
-- p_force now bypasses time-window checks only; it no longer adds current-2.
...
'Dispatches automatic AT sync jobs for current + previous year only. p_force bypasses time-window checks only.';
