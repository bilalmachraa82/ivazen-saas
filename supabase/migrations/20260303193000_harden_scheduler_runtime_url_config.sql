-- Harden scheduler: remove hardcoded process queue URL from SQL function
-- and load it from runtime config table.

CREATE TABLE IF NOT EXISTS public.at_sync_runtime_config (
  id boolean PRIMARY KEY DEFAULT true,
  process_queue_url text NOT NULL,
  updated_at timestamptz NOT NULL DEFAULT now(),
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT at_sync_runtime_config_singleton CHECK (id = true)
);

INSERT INTO public.at_sync_runtime_config (id, process_queue_url)
VALUES (true, 'https://dmprkdvkzzjtixlatnlx.supabase.co/functions/v1/process-at-sync-queue')
ON CONFLICT (id) DO NOTHING;

ALTER TABLE public.at_sync_runtime_config ENABLE ROW LEVEL SECURITY;

REVOKE ALL ON public.at_sync_runtime_config FROM PUBLIC;
GRANT SELECT ON public.at_sync_runtime_config TO service_role;
...
'Hardened scheduler: queue current+previous year only, runtime URL from at_sync_runtime_config, no hardcoded endpoint in function body.';
