-- Fix: Scheduler re-enqueues retryable error jobs but never triggers the
-- processor for their OLD batch IDs.  process-at-sync-queue requires a
-- batchId, so re-enqueued jobs sit in 'pending' forever, also blocking new
-- job creation (the NOT EXISTS check prevents new jobs for those clients).
--
-- Solution: After re-enqueuing, collect the DISTINCT old batch IDs and fire
-- net.http_post for each one so the processor picks them up.
...
BEGIN
  ...
  -- FIX: Trigger the processor for EACH old batch that has re-enqueued pending jobs.
  -- Without this, re-enqueued jobs sit in 'pending' forever because
  -- process-at-sync-queue requires a batchId and only new batches get triggered below.
  IF v_retried > 0 THEN
...
'Dispatches automatic AT sync jobs. Default years: current + previous. Runs at PT local windows (19:30 and 06:00). FIX 2026-03-19: triggers processor for re-enqueued retry batches.';
