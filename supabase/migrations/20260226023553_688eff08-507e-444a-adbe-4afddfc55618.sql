...
-- Consolidate at_withholding_candidates schema (retry without problematic RAISE block)
...
UPDATE public.at_withholding_candidates SET confidence_score = confidence WHERE confidence_score IS NULL AND confidence IS NOT NULL;
UPDATE public.at_withholding_candidates SET confidence = confidence_score WHERE confidence IS NULL AND confidence_score IS NOT NULL;
UPDATE public.at_withholding_candidates SET confidence_score = 0 WHERE confidence_score IS NULL;
UPDATE public.at_withholding_candidates SET confidence = confidence_score WHERE confidence IS NULL;
UPDATE public.at_withholding_candidates SET sync_history_id = source_sync_history_id WHERE sync_history_id IS NULL AND source_sync_history_id IS NOT NULL;
UPDATE public.at_withholding_candidates SET source_sync_history_id = sync_history_id WHERE source_sync_history_id IS NULL AND sync_history_id IS NOT NULL;
UPDATE public.at_withholding_candidates SET detected_keys = '{}'::text[] WHERE detected_keys IS NULL;
UPDATE public.at_withholding_candidates SET raw_payload = '{}'::jsonb WHERE raw_payload IS NULL;

UPDATE public.at_withholding_candidates c
SET accountant_id = cred.accountant_id
FROM public.at_credentials cred
WHERE c.accountant_id IS NULL AND cred.client_id = c.client_id AND cred.accountant_id IS NOT NULL;

WITH ranked AS (
  SELECT ca.client_id, ca.accountant_id,
...
FROM ranked r
WHERE c.accountant_id IS NULL AND c.client_id = r.client_id AND r.rn = 1;

UPDATE public.at_withholding_candidates SET status = 'pending' WHERE status IS NULL OR btrim(status) = '';
UPDATE public.at_withholding_candidates
SET status = 'skipped', notes = trim(both ' ' FROM concat_ws(' | ', notes, 'Status normalized during consolidation'))
WHERE status NOT IN ('pending', 'promoted', 'rejected', 'skipped');

WITH ranked AS (
  SELECT id, row_number() OVER (PARTITION BY client_id, beneficiary_nif, document_reference, fiscal_year ORDER BY created_at ASC, id ASC) AS rn
  FROM public.at_withholding_candidates WHERE document_reference IS NOT NULL
)
DELETE FROM public.at_withholding_candidates c USING ranked r WHERE c.id = r.id AND r.rn > 1;
...
BEGIN
  FOR v_conname IN
    ...
    WHERE conrelid = 'public.at_withholding_candidates'::regclass AND contype = 'c' AND pg_get_constraintdef(oid) ILIKE '%status%'
  LOOP
    EXECUTE format('ALTER TABLE public.at_withholding_candidates DROP CONSTRAINT IF EXISTS %I', v_conname);
  END LOOP;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'public.at_withholding_candidates'::regclass AND conname = 'at_withholding_candidates_status_check') THEN
    EXECUTE 'ALTER TABLE public.at_withholding_candidates ADD CONSTRAINT at_withholding_candidates_status_check CHECK (status IN (''pending'', ''promoted'', ''rejected'', ''skipped''))';
  END IF;
END $do$;

ALTER TABLE public.at_withholding_candidates ALTER COLUMN status SET DEFAULT 'pending';

CREATE INDEX IF NOT EXISTS idx_at_withholding_candidates_client_status ON public.at_withholding_candidates (client_id, status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_at_withholding_candidates_confidence ON public.at_withholding_candidates (client_id, confidence_score DESC, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_at_withholding_candidates_sync ON public.at_withholding_candidates (sync_history_id, client_id, created_at DESC);

DO $do$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'at_withholding_candidates'
...
CREATE POLICY at_withholding_candidates_select_access
  ON public.at_withholding_candidates FOR SELECT TO authenticated
  USING (
    client_id = auth.uid()
    OR EXISTS (SELECT 1 FROM public.client_accountants ca WHERE ca.client_id = at_withholding_candidates.client_id AND ca.accountant_id = auth.uid())
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_updated_at_column' AND pronamespace = 'public'::regnamespace) THEN
    DROP TRIGGER IF EXISTS trg_at_withholding_candidates_updated_at ON public.at_withholding_candidates;
    DROP TRIGGER IF EXISTS update_at_withholding_candidates_updated_at ON public.at_withholding_candidates;
    CREATE TRIGGER trg_at_withholding_candidates_updated_at BEFORE UPDATE ON public.at_withholding_candidates FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
  END IF;
END $do$;
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.tax_withholdings (client_id,fiscal_year,beneficiary_nif,beneficiary_name,income_category,income_code,location_code,gross_amount,exempt_amount,dispensed_amount,withholding_rate,withholding_amount,payment_date,document_reference,source_sales_invoice_id,status,notes,is_non_resident)
    VALUES (v_candidate.client_id,v_candidate.fiscal_year,v_candidate.beneficiary_nif,v_candidate.beneficiary_name,v_candidate.income_category,v_candidate.income_code,'C',coalesce(v_candidate.gross_amount,0),0,0,v_candidate.withholding_rate,coalesce(v_candidate.withholding_amount,0),v_candidate.payment_date,v_candidate.document_reference,v_candidate.source_sales_invoice_id,'draft',trim(both ' ' FROM concat_ws(' | ','AT candidate promoted',nullif(v_candidate.detection_reason,''))),false)
