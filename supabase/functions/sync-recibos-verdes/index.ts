...
type PortalFallbackPartition = {
  acceptedRecords: RecibosRecord[];
  rejectedCount: number;
  rejectedDocumentTypes: Record<string, number>;
};
...
function isReciboVerdeDocumentType(
...
function normalizePortalDocumentType(
  documentType: string | null | undefined,
): string {
  const normalized = String(documentType || "").trim().toUpperCase();
...
export function partitionPortalFallbackRecords(
  records: RecibosRecord[],
): PortalFallbackPartition {
  const acceptedRecords: RecibosRecord[] = [];
  const rejectedDocumentTypes: Record<string, number> = {};

  for (const record of records || []) {
    if (isReciboVerdeDocumentType(record.documentType)) {
      acceptedRecords.push(record);
      continue;
    }

    const normalizedType = normalizePortalDocumentType(record.documentType);
    rejectedDocumentTypes[normalizedType] =
      (rejectedDocumentTypes[normalizedType] || 0) + 1;
  }

  return {
    acceptedRecords,
    rejectedCount: Math.max(0, (records?.length || 0) - acceptedRecords.length),
...
function buildConnectorFetchInit(
  ...
  if (caCertPem) {
    try {
      ...
      console.warn(
        "[sync-recibos] Failed to create custom HTTP client:",
        error?.message,
      );
    }
  }
  ...
  Deno.serve(async (req: Request) => {
    ...
    try {
      ...
      if (!isServiceRole) {
        ...
        const { data: { user }, error: authError } = await authSupabase.auth
          .getUser();
        if (authError || !user) {
          return new Response(
        ...
        if (!isWithinATWindow()) {
          return new Response(
            JSON.stringify({
              ...
              message:
                "Fora da janela horária AT (19:00-06:00). Tente mais tarde.",
            }),
            {
              status: 200,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      if (
        ...
        return new Response(
          JSON.stringify({
            success: false,
            error: "AT connector not configured",
          }),
          {
            status: 500,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      const officialAttempts: Array<
        { source: string; username: string; password: string }
      > = [];
      ...
      try {
        if (
          ...
          for (let index = 0; index < officialAttempts.length; index++) {
            ...
            console.log(
              ...
              `username_kind=${
                isLikelyWfaUsername(attempt.username) ? "wfa" : "nif"
              }`,
            );
            ...
            try {
              ...
              officialData = {
                success: false,
                error: `Connector returned non-JSON: ${
                  officialText.slice(0, 200)
                }`,
              };
            }

            if (officialData.success && officialData.vendas?.success) {
              const recibos = (officialData.vendas.invoices || []).filter((
                invoice,
              ...
              if (recibos.length > 0) {
                ...
                return new Response(
                  JSON.stringify({
                    ...
                    message:
                      `Imported ${result.inserted} recibos verdes via webservice oficial`,
                  }),
                  {
                    status: 200,
                    headers: {
              ...
              const vendasError = officialData.vendas?.errorMessage ||
                officialData.error;
        ...
        if (
          !portalConnectorUrl || !portalConnectorToken || !portalNif ||
          !portalPassword
        ) {
          if (
            attemptedOfficialConnector && officialReturnedEmpty &&
            !lastOfficialError
          ) {
            await supabase
              .from("at_credentials")
        ...
        console.log(
          "[sync-recibos] Connector response:",
          connResp.status,
          connText.slice(0, 500),
        );
        ...
        try {
          ...
          connData = {
            success: false,
            error: `Connector returned non-JSON: ${connText.slice(0, 200)}`,
          };
        }
        ...
        if (!connData.success) {
          ...
          return new Response(
            JSON.stringify({
              success: false,
              reasonCode: connData.error?.includes("invalid")
                ? "AT_AUTH_FAILED"
                : "UNKNOWN_AT_ERROR",
        ...
        const partitionedRecords = partitionPortalFallbackRecords(records);
        const connectorDebug = {
          ...
          acceptedReciboRecords: partitionedRecords.acceptedRecords.length,
          rejectedNonReciboRecords: partitionedRecords.rejectedCount,
          rejectedDocumentTypes:
            Object.keys(partitionedRecords.rejectedDocumentTypes).length > 0
              ? partitionedRecords.rejectedDocumentTypes
              : null,
        };
        ...
        if (partitionedRecords.acceptedRecords.length === 0) {
          console.warn(
            "[sync-recibos] Portal fallback returned only non-recibo document types",
            partitionedRecords.rejectedDocumentTypes,
          );
          ...
          return new Response(
            JSON.stringify({
              ...
              reasonCode: "AT_EMPTY_LIST",
              message:
                "Portal fallback returned no recibos verdes for this period",
              totalRecords: 0,
              rawRecords: records.length,
              eligibleRecords: 0,
              filteredOut: partitionedRecords.rejectedCount,
              rejectedDocumentTypes: partitionedRecords.rejectedDocumentTypes,
              inserted: 0,
              skipped: 0,
              errors: 0,
              connectorDebug,
        ...
        if (partitionedRecords.rejectedCount > 0) {
          console.warn(
            "[sync-recibos] Ignoring non-recibo portal records",
            partitionedRecords.rejectedDocumentTypes,
          );
        }

        for (const rec of partitionedRecords.acceptedRecords) {
          ...
          const { error: insertError } = await supabase
            ...
            .insert({
              ...
              image_path: `at-portal-recibos/${clientId}/${
                documentNumber || Date.now()
              }`,
              status: "validated",
              validated_at: new Date().toISOString(),
        ...
        return new Response(
          JSON.stringify({
            success: true,
            totalRecords: partitionedRecords.acceptedRecords.length,
            rawRecords: records.length,
            eligibleRecords: partitionedRecords.acceptedRecords.length,
            filteredOut: partitionedRecords.rejectedCount,
            rejectedDocumentTypes:
              Object.keys(partitionedRecords.rejectedDocumentTypes).length > 0
                ? partitionedRecords.rejectedDocumentTypes
            ...
            errors,
            message:
              `Imported ${inserted} recibos verdes (${skipped} duplicates skipped${
                partitionedRecords.rejectedCount > 0
                  ? `, ${partitionedRecords.rejectedCount} non-recibo filtered out`
                  : ""
              })`,
            connectorDebug,
          }),
