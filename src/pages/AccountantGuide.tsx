...
const buildSections = (): GuideSection[] => {
  ...
  },
  {
    ...
    badge: 'Novo',
    items: [
      { text: 'Regime IVA → Definições → Editar cliente → "Cadência IVA" (mensal ou trimestral)', link: '/settings' },
      { text: 'Tipo de contribuinte → ENI, Empresa ou Misto — determina obrigações visíveis' },
      { text: 'Credenciais AT → Definições → separador "Credenciais AT" — NIF + password do portal' },
      { text: 'O regime IVA afecta: prazos fiscais, alertas, e períodos do apuramento' },
      { text: 'Clientes Art. 53 (isentos) → configurar como trimestral; SS não se aplica', highlight: true },
    ],
  },
  {
    id: 'validacao',
    title: 'Validação de compras — dicas práticas',
    icon: CheckCircle,
    badge: 'Novo',
    items: [
      { text: 'Priorizar recém-importadas → usar filtro "Últimas 24h" ou "Últimos 7 dias"', link: '/validation?recent=24h' },
      { text: 'Excluir em massa → Selecionar → escolher facturas → "Não contabilizar" (exclui do IVA sem apagar)', link: '/validation' },
      { text: 'Fornecedor só mostra NIF → normal para importações AT; nome é enriquecido automaticamente' },
      { text: 'Sem imagem na fatura → esperado para facturas importadas da AT (SIRE/CSV)' },
      { text: 'NIF 999999990 = consumidor final → impossível resolver nome', highlight: true },
      { text: 'Duplicados → usar separador "Duplicados" → sistema mantém cópia com imagem (uploaded > AT)' },
      { text: 'Re-classificar → selecionar facturas → "Reclassificar" → IA re-analisa' },
    ],
  },
  {
    id: 'reconciliacao',
    title: 'Reconciliação AT',
    icon: ArrowLeftRight,
    items: [
      { text: '1. Abrir Reconciliação → cruzamento automático AT vs app', link: '/reconciliation' },
      { text: '2. Separador "AT" → facturas importadas da Autoridade Tributária' },
      { text: '3. Separador "Divergências" → facturas que não correspondem (valor ou NIF diferente)' },
      { text: '4. Nomes dos fornecedores aparecem quando disponíveis no directório' },
      { text: 'Esta secção é apenas de consulta — acções de validação fazem-se em Compras/Vendas', highlight: true },
    ],
  },
  {
    id: 'vazio',
  ...
  },
  {
    ...
    icon: Lightbulb,
    items: [
      { text: 'Porque é que as vendas não aparecem? → Vendas (recibos verdes) são importação manual — descarregar Excel do portal AT' },
      { text: 'Posso eliminar facturas? → "Eliminar" remove do sistema; "Excluir" (Não contabilizar) mantém mas ignora no IVA' },
      { text: 'O Dashboard mostra valores mas o Centro Fiscal está a zeros? → Importar os dados em falta' },
      { text: 'Alerta de IVA mensal para cliente trimestral? → Editar cliente → mudar Cadência IVA para trimestral', link: '/settings' },
      { text: 'Porque é que ao limpar duplicados a imagem desapareceu? → Sistema mantém a cópia uploaded; re-importar a fatura' },
    ],
  },
];
};
